...
class BgEditor extends PureComponent {
  ...
  render() {
    ...
    return (
      <React.Fragment>
        {/*背景图栏*/}
        <Card title={"背景图"} className={"bg_editor_card"} extra={<div onClick={this.showModal}>
          <Icon type="plus"/>
          上传图片
        </div>}>
          {/*判断背景图是否存在*/}
          {$$img ? (
            ...
            // 不存在
            <div onClick={this.showModal} className={"bg_editor-background_transparent"}/>
          )}
          <UpImgPart
        ...
        <Card title={"背景色配置"} className={"bg_editor_card"}>
          ...
          />} trigger="click" placement="left">
            <Card.Grid className={"bg_editor_card-grid"} style={{ background: $$color }}>
              <Icon type="edit"/>&nbsp;&nbsp;自定义
            </Card.Grid>
          </Popover>
        </Card>
