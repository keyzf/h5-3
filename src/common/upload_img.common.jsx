...
import { upload_ajax } from "../ajax/upload";
...
export default class UploadImgCommon extends PureComponent {
  ...
  uploadChange = changedFields => {
    ...
    if (file.status === "done") {
      upload_ajax(1, "个人资源", `http://src.e7wei.com/${file.response.key}`)
        .then(ajax_data => {
          NProgress.done();
          NProgress.remove();
          ...
          this.props.upData("UPLOAD_RECODE", {
            user_upload_img: $$data
              .get("user_upload_img")
              .unshift(fromJS({ url: `http://src.e7wei.com/${file.response.key}`, mid: ajax_data.mid })),
