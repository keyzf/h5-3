...
class App extends PureComponent {
   ...
   *     五、http://localhost:3000?state=release&id=5&number=2 (发布页展示)
   */
  componentWillMount() {
     ...
     */
    if (user_id) {
      upDate('USER_ID', { user_id: user_id, number: number });
    }
    /**
     * 页面跳转至visual
      ...
      if ($$api.get('surroundings') === 'development') {
        axios
          ...
          .then(response => {
            ...
            upDate('RELEASE', { url: response.data.release });
            this.setState({
              router: 'visual',
            });
          });
