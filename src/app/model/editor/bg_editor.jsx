...
class BgEditor extends React.Component {
  ...
  bg_img_config = (tip, e) => {
    ...
    if (tip === 'tiling') {
      $$bg_new = this.props.bg_value.data.setIn(
        ['customize', 'img_config', 'tiling'],
        e.target.checked
      );
    } else {
      $$bg_new = this.props.bg_value.data.setIn(
        ['customize', 'img_config', 'locking'],
        e.target.checked
      );
    }
    this.props.bg_upData($$bg_new, ''.false);
  ...
  render() {
    ...
    const $$bg_img_config = this.props.bg_value.data
      .get('customize')
      .get('img_config');
    return (
      <div>
          ...
          $$bg_img ? (
            ...
            <div style={{ width: '100%', height: '100%' }}>
              ...
              >
                ...
                <Checkbox
                  onChange={this.bg_img_config.bind(this, 'tiling')}
                  defaultChecked={$$bg_img_config.get('tiling')}
                >
                  平铺
                </Checkbox>
                <Checkbox
                  onChange={this.bg_img_config.bind(this, 'locking')}
                  defaultChecked={$$bg_img_config.get('locking')}
                >
                  固定
                </Checkbox>
              </div>
