...
class EditorImg extends React.Component {
  ...
  editorFeatures = (opt_name, data) => {
    ...
    if (opt_name === "item_deletes") {
      const $$new_data = $$select_data.get($$choose_data.get("number")).get("customize").get("item").delete(data);
    ...
    if (opt_name === "add_item") {
      const $$add = $$select_data.get($$choose_data.get("number")).get("customize").get("item").push(fromJS($$img_add_item));
      this.sendAction($$select_data.get($$choose_data.get("number")).setIn(["customize", "item"], $$add));
    }
    if (opt_name === "base") {
      if (data.layout) {
        this.sendAction(
      ...
      if (data.show_element) {
        this.sendAction(
          $$select_data
            .get($$choose_data.get("number"))
            .setIn(["customize", "base", "show_element", "value"], data.show_element.value)
        );
      }
    }
    if (opt_name === "color") {
    ...
    if (opt_name === "tiling") {
      this.sendAction(
        $$select_data
          .get($$choose_data.get("number"))
          .setIn(["advance", "img_config", "tiling", "value"], data.target.checked)
      );
    }
    if (opt_name === "stretching") {
      this.sendAction(
        $$select_data
          .get($$choose_data.get("number"))
          .setIn(["advance", "img_config", "stretching", "value"], data.target.checked)
      );
    }
    ...
    if (opt_name === "item_tiling") {
      this.sendAction(
        $$select_data
          .get($$choose_data.get("number"))
          .setIn(["customize", "item", this.state.number, "img_config", "tiling", "value"], data.target.checked)
      );
    }
    if (opt_name === "item_stretching") {
      this.sendAction(
        $$select_data
          .get($$choose_data.get("number"))
          .setIn(["customize", "item", this.state.number, "img_config", "stretching", "value"], data.target.checked)
      );
    }
  ...
  render() {
    ...
    const operations = (<Button onClick={this.editorFeatures.bind(this, "reset")}>恢复默认</Button>);
    const item = (img, title, number) => (<Row gutter={16}>
      <Col span={8}><img width={"50px"} height={"auto"}
                         src={img ? img : "http://demos.creative-tim.com/material-kit-pro/assets/img/image_placeholder.jpg"}
                         alt={"img"}/></Col>
      <Col span={12} style={{ padding: "8px" }}>{title}</Col>
      <Col span={4} style={{ padding: "8px" }}>
        <Tooltip title="修改">
          <Icon type="edit" style={{ marginRight: "15px" }} onClick={this.changeItem.bind(this, number)}/>
        </Tooltip>
        <Tooltip title="删除">
          <Icon type="delete" onClick={this.editorFeatures.bind(this, "item_deletes", number)}/>
        </Tooltip>
      </Col>
    </Row>);
    return (
      <div>
        {
          this.state.item ?
            <Card title="编辑图集数据" extra={<div onClick={this.backItem}>返回</div>}>
              <Row gutter={16}>
                <Col span={7} offset={3}
                     style={{
                       margin: "auto",
                ...
                >
                  <img
                    ...
                    src={
                      $$customize.get("item").get(this.state.number).get("img")
                        ? $$customize.get("item").get(this.state.number).get("img")
                ...
                <Col span={14}>
                  <Button.Group>
                    ...
                    <Button onClick={this.editorFeatures.bind(this, "item_delete")}>
                      删除
                    </Button>
                  </Button.Group>
                  <br/>
                  ...
                  <Row gutter={16}>
                    <Col span={10}>
                      <Checkbox
                        ...
                        defaultValue={$$customize.get("item").get(this.state.number).get("img_config").get("tiling").get("value")}
                      >
                        平铺
                      </Checkbox>
                    </Col>
                    <Col span={12}>
                      <Checkbox
                        onChange={this.editorFeatures.bind(this, "item_stretching")}
                        defaultValue={$$customize.get("item").get(this.state.number)
                          .get("img_config")
                          .get("stretching").get("value")}
                      >
                        拉伸
                      </Checkbox>
                    </Col>
                  ...
                  <UpImgPart
                    ...
                    img={$$customize.get("item").get(this.state.number).get("img")}
                  />
                </Col>
              </Row>
              <Divider/>
            ...
            <Tabs defaultActiveKey={"1"} tabBarExtraContent={operations} key={1}>
              ...
              <Tabs.TabPane tab="内容设置" key="1">
                {$$customize.get("name") === "up_img" ? "" : <Card title="基础属性" style={{ marginTop: "-18px" }}>
                  <ImgBaseForm name={$$customize.get("name")} {...$$customize.get("base").toJS()}
                               onChange={this.editorFeatures.bind(this, "base")}/>
                </Card>}
                <Card title="项目列表" extra={<div onClick={this.editorFeatures.bind(this, "add_item")}><Icon
                  type="plus"/>添加</div>}>
                  {
                    $$customize.get("item").map((data, index) => {
                      return <Collapse.Panel style={{ marginBottom: "10px" }} disabled
                      ...
                      <Row gutter={16}>
                        <Col span={10}>
                          <Checkbox
                            ...
                            defaultValue={$$advance.get("img_config").get("tiling").get("value")}
                          >
                            平铺
                          </Checkbox>
                        </Col>
