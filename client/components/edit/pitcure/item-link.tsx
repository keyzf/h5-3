...
const PictureItemLink = React.memo((props: { index: number }) => {
  ...
  const type = ui[editList[0]].base[props.index].link.type;
  const url = ui[editList[0]].base[props.index].link.url;
  return (
    <React.Fragment>
      <Form.Item label="点击跳转" {...formItemLayout}>
        <Select value={type} onChange={onSelect}>
          <Select.Option value="choose">请选择</Select.Option>
          <Select.Option value="web">点击跳转</Select.Option>
...
export default PictureItemLink;
