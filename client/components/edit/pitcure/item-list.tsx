...
class ItemList extends React.PureComponent<any, State> {
  ...
  handleMouseMove = ({ pageY }:any) => {
    ...
    if (isPressed) {
      ...
      const currentRow = clamp(Math.round(mouseY / 20), 0, 100 - 1);
      let newOrder = order;

      if (currentRow !== order.indexOf(originalPosOfLastPressed)) {
        newOrder = reinsert(order, order.indexOf(originalPosOfLastPressed), currentRow);
      }
      this.props.changeFun({ type: "PICTURE_BASE", payload: { base: newOrder } });
              ...
              <Motion style={style} key={index}>
                {({ scale, shadow, y }) =>
                  <div
                    ...
                    <Col span={8} style={{ userSelect: "none", pointerEvents: "none" }}>
                      <img
                        width="50px"
                        height="40px"
                        src={`${i.crop}`}
                    ...
                    <Col span={6} style={{ padding: "10px 10px", userSelect: "none", pointerEvents: "none" }}>
                      <span>{index + 1}</span>
                    </Col>
